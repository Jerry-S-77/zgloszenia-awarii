...
8a. **awarie_zdjecia** — do 3 zdjęć na awarię; plik JPEG w prywatnym kubełku Storage `zdjecia-awarii` (ścieżka `<awaria>/<zdjęcie>.jpg`, do 2 MB). Widzi i dodaje ten, kto widzi awarię (zgłaszający swoją, obsługa każdą); usuwa autor albo admin. Autor i limit pilnowane w bazie (trigger i polityka Storage), dodanie i usunięcie trafia do historii.
...
3. **Zgłoszenie awarii** (`/`) — wybór urządzenia z listy (autouzupełnienie kategorii/krytyczności), data/godzina (domyślnie teraz), opis, krytyczność skutku, opcjonalnie do 3 zdjęć (zmniejszanych w telefonie; offline czekają w kolejce razem ze zgłoszeniem). Osoba zgłaszająca to zalogowane konto. Status ustawiany automatycznie na „Zgłoszona”; numer nadaje baza po zapisaniu (zgłoszenie z kolejki offline pokazuje „oczekuje na numer” do czasu synchronizacji).
...
5. **Karta awarii** (`/awarie/$id`) — oś statusów, przycisk z kolejnym dozwolonym krokiem dla roli przeglądającego (przyjęcie, rozpoczęcie/wstrzymanie naprawy, zamknięcie z przyczyną i czasem przestoju, ponowne otwarcie tylko kierownik/admin), sekcja „Zdjęcia” (miniatury, podgląd, dodawanie także offline, usuwanie przez autora lub admina), sekcja „Zespół” (lista osób, „Dołącz do zadania” / „Opuść zadanie”, kierownik i admin dodają i usuwają osoby; wymaga połączenia), komentarze i pełna historia zmian. Zapis niesie oczekiwaną wersję rekordu — przy konflikcie (ktoś inny zmienił zgłoszenie w międzyczasie) pokazuje komunikat i odświeża dane, nie nadpisuje cudzej zmiany.
